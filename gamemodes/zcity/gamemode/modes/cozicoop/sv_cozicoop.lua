...
function MODE:GetLootTable()
    ...
    local mapData = self.Maps[currentMap] or {PlayerEqipment = "cozirebel"}

    local lootData = mapData.PlayerEqipment == "cozirebel" and self.LootTable[2] or self.LootTable[1]
    return lootData[2] 
end
                ...
                if restored and data then
                    ...
                    elseif savedSubClass == "medic" then
                        ...
                        if savedPlayerClass == "Refugee" then
                            ...
                            ply:SetPlayerClass(savedPlayerClass or "CoziRebel", {bNoEquipment = true})
                        end
                    ...
                    elseif savedSubClass == "grenadier" then
                        ...
                        if savedPlayerClass == "Refugee" then
                            ...
                            ply:SetPlayerClass(savedPlayerClass or "CoziRebel", {bNoEquipment = true})
                            zb.GiveRole(ply, savedRole or "CoziRebel", savedRoleColor)
                        end
                    end
...
function MODE:GiveDefaultEquipment(ply, playerClass, hasGordon, medicCount, maxMedics, grenadierCount, maxGrenadier, savedGordonExists)
    ...
    if not hasGordon and not ply:IsBot() and not savedGordonExists then
        ...
        zb.GiveRole(ply, "CoziFreeman", clr_rebel)
        wasGordon = true
    else
        local isMedic = false
        ...
        if playerClass == "refugee" or playerClass == "citizen" then
            ply:SetPlayerClass("Refugee", {bNoEquipment = playerClass == "citizen"})
        elseif playerClass == "cozirebel" then
            ply:SetPlayerClass("cozirebel")
        end
        ...
        elseif playerClass == "cozirebel" then
            zb.GiveRole(ply, "cozirebel", clr_rebel)
        end
    end
...
local function PossessNPC(ply, npc)
    ...
    local currentMap = game.GetMap()
    local mapData = CurrentRound().Maps[currentMap] or {PlayerEqipment = "cozirebel"}
    local playerClass = mapData.PlayerEqipment
