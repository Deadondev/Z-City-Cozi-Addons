...
SWEP.PrintName = "Makeshift Bandage"
SWEP.Instructions = "Whatever you can muster to stop bleeding. RMB to use on someone else."
...
function SWEP:Think()
	...
	if self:GetClass() == "weapon_makeshiftbandage_sh" then
	...
	if not self:GetOwner():KeyDown(IN_ATTACK) and self.modeValues[1] > 0 then --bandage degeneration
...
function SWEP:PrimaryAttack()
	if SERVER then--and not self.modeValuesdef[self.mode][2] then
			if self.modeValues[1] < 0 then --if silly bullshit happens this stops it.
			self.modeValues[1] = 0;
			end
		self.modeValues[1] = self.modeValues[1] + 0.05; --Makeshift part
...
function SWEP:SecondaryAttack()
	...
	if SERVER  and not self:GetOwner():KeyDown(IN_ATTACK) and self:GetOwner():KeyDown(IN_ATTACK2) then
		self.modeValues[1] = self.modeValues[1] + 0.1; --Makeshift part but for FRIENDS its 2x BETTTERRRRR! (the idea is to simulate how someone else can tend your injuries better, and also promote teamwork
