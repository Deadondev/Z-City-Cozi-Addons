...
local function Initialize()
    ...
    local RESPAWN_TIME            = 25    -- wave countdown in seconds, was 25
    local GORDON_EXCLUSION_ROUNDS = 2     -- rounds before last-Gordon can be Gordon again
    local RESPAWN_LIVES           = 10    -- how many lives you start with
    playerlives = 10 --this is here bc idk its where it needs to be defined
    -- ── Gordon helpers ────────────────────────────────────────────────────────────
    ...
    hook.Add("PlayerDeath", "ZCity_CoopRespawn", function(victim)
        print("[ZC Respawn] PlayerDeath: " .. victim:Nick() ..
            ...
            if playerlives < 1 then -- MY BULLSHIT
            print("[ZC Respawn]   -> SKIP: No lives left!")
        --    victim:Team() = TEAM_SPECTATOR
            return
        end
        ...
        if not waveTimerActive then
            ...
            timer.Create("ZC_REBEL_WAVE", RESPAWN_TIME, 1, function()
                ...
                for _, ply in ipairs(rebelWaveQueue) do
                    ...
                    playerlives = playerlives - waveSpawnCount --TAKING A LIFE.
                    print("[ZC Respawn]   -> Lives are now at: (" .. playerlives .. "s)") --print("[ZC Respawn]   -> Lives are now at: " (" .. playerlives .. "s))
                end
